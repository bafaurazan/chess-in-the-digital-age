...
ggplot(Data_games_count, aes(x = rev(1:nrow(my_games_count)), y = count)) +
  ...
  # strzalka
  geom_segment(aes(70, 58000000, xend = 86, yend = 62000000),
               linewidth=1.2,
...
# Wpływ serialu "Gambit królowej" na świat szachów

#### Wyszukiwanie Ilości zagranych partii z debiutem "Gambit królowej"
...
ggplot(Data_Queens_gambit_games_count, aes(x = rev(1:nrow(my_games_count)), y = count)) +
...
for(i in 136:length(files_to_read)){
  ...
  cow_moves = stri_match_all(pgn.df[moves,], regex = "1. \\s*(.*?)\\s*7. ")[[1]]
  
  cow_moves = sub("\\[(\\b+).+", "\\1", pgn.df[moves,]) 
  
  
  All_games_count_v2 = length(pgn.df[elo_index_v2,])
  
  my_novice_games_count_v2 = length(which(as.numeric(pgn.df[elo_index_v2,]) < 1100) == TRUE)
